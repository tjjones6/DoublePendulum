%% Double Pendulum Simulation (IC Sensitivity)
% Author:      Tyler Jones
...
% Initial Conditions for first pendula
...
% Initial Conditions for second pendula
epsilon = 0.01; % Small perturbation to initial data
theta3_0 = pi/2;  % Initial angle of the first pendulum (radians)
theta4_0 = (1-epsilon)*pi/2;  % Initial angle of the second pendulum (radians)
omega3_0 = 0;      % Initial angular velocity of the first pendulum (rad/s)
omega4_0 = 0;      % Initial angular velocity of the second pendulum (rad/s)

% Time Settings
tspan = [0 50];  % Extend the time span for simulation (seconds)

%% Numerical Integration using ODE45
options = odeset('RelTol', 1e-6, 'AbsTol', 1e-6);
[t1, Y1] = ode45(@(t, y) doublePendulumODE(t, y, g, L1, L2, m1, m2), tspan, [theta1_0, omega1_0, theta2_0, omega2_0], options);
[t2, Y2] = ode45(@(t, y) doublePendulumODE(t, y, g, L1, L2, m1, m2), tspan, [theta3_0, omega3_0, theta4_0, omega4_0], options);
...
myWriter = VideoWriter('DoublePendulumPerturburbationAnimation.mp4', 'MPEG-4');
...
for i = 1:length(t1)
    % Plot the trajectory
    plot(x2(1:i), y2(1:i), 'Color',[1 0 0 0.3])
    if i > 20
        plot(x2(i-20:i), y2(i-20:i), '-r')
    end

    % Plot the trajectory
    plot(x4(1:i), y4(1:i), 'Color',[0 0 1 0.3])
    if i > 20
        plot(x4(i-20:i), y4(i-20:i), '-b')
    end
    ...
    plot(x1(i), y1(i), 'ko', 'MarkerSize', 10, 'MarkerFaceColor', 'k');  % Mass for Pendulum 1
    plot(x2(i), y2(i), 'ro', 'MarkerSize', 10, 'MarkerFaceColor', 'r');  % Mass for Pendulum 2

     % Update the position of the pendulum bobs and rods in the animation plot
    plot([0, x3(i)], [0, y3(i)], 'k', 'LineWidth', 2);  % Rod for Pendulum 1
    hold on;
    plot([x3(i), x4(i)], [y3(i), y4(i)], 'k', 'LineWidth', 2);  % Rod for Pendulum 2
    plot(x3(i), y3(i), 'ko', 'MarkerSize', 10, 'MarkerFaceColor', 'k');  % Mass for Pendulum 1
    plot(x4(i), y4(i), 'bo', 'MarkerSize', 10, 'MarkerFaceColor', 'b');  % Mass for Pendulum 2


    fig_xytit('x-axis','y-axis',['Double Pendulum Animation - Time: ', num2str(t1(i), '%.2f'), 's'])
